...
class RentalModel extends Model
{
    ...
    public function getLastTenRental()
    {
        $rentals = [];
    ...
    public function deleterental(int $id)
    {
        ...
        try {
            ...
            $req = $this->getDb()->prepare('DELETE FROM `chat` WHERE `id_rental` = :id_rental');
            $req->bindParam('id_rental', $id, PDO::PARAM_INT);
            $req->execute();

            // Delete links in picture table
            $req = $this->getDb()->prepare('DELETE FROM `picture` WHERE `id_rental` = :id_rental');
            $req->bindParam('id_rental', $id, PDO::PARAM_INT);
            $req->execute();

            // Delete links in reservation table
            $req = $this->getDb()->prepare('DELETE FROM `reservation` WHERE `id_rental` = :id_rental');
            $req->bindParam('id_rental', $id, PDO::PARAM_INT);
            $req->execute();

            // Delete links in review table
            $req = $this->getDb()->prepare('DELETE FROM `review` WHERE `id_rental` = :id_rental');
            $req->bindParam('id_rental', $id, PDO::PARAM_INT);
            $req->execute();

            // Delete links in wishlist table
            $req = $this->getDb()->prepare('DELETE FROM `wishlist` WHERE `id_rental` = :id_rental');
            $req->bindParam('id_rental', $id, PDO::PARAM_INT);
            $req->execute();

            // Delete links in rental_category table
            $req = $this->getDb()->prepare('DELETE FROM `rental_category` WHERE `id_rental` = :id_rental');
            $req->bindParam('id_rental', $id, PDO::PARAM_INT);
            $req->execute();

            // Delete the rental
            $req = $this->getDb()->prepare('DELETE FROM `rental` WHERE `id_rental` = :id_rental');
            $req->bindParam('id_rental', $id, PDO::PARAM_INT);
            $req->execute();
